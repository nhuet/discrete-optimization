...
def test_makespan_constraint(problem, solver, random_seed):
    sol: MultiskillRcpspSolution = solver.solve(
        callbacks=[NbIterationStopper(nb_iteration_max=1)]
    ).get_best_solution()
    print(sol.get_max_end_time())
    makespan_max = 900

    # before adding the constraint, not already satisfied
    assert sol.get_max_end_time() > makespan_max
    # add constraint: should be now satisfied
    cstrs = solver.add_constraint_on_max_end_time(sign=SignEnum.LEQ, time=makespan_max)
    sol = solver.solve(
        callbacks=[NbIterationStopper(nb_iteration_max=1)]
    ).get_best_solution()
    assert sol.get_max_end_time() <= makespan_max
