...
def test_mode_constraint_multimode(random_seed):
    ...
    mode = 1
    for _ in range(5):
        # at least 1 of 5 solution has a different mode
        sol: RcpspSolution = solver.solve(
            callbacks=[NbIterationStopper(nb_iteration_max=1)]
        ).get_best_solution()
...
def test_max_end_time_constraint():
    model = "j301_1.sm"
    files_available = get_data_available()
    file = [f for f in files_available if model in f][0]
    problem = parse_file(file)
    print(problem.horizon)
    solver = CpSatRcpspSolver(problem=problem)
    res = solver.solve(callbacks=[NbIterationStopper(nb_iteration_max=2)])
    print([fit for sol, fit in res])
    max_time_target = 43
    min_time_target = 80
    sol: RcpspSolution = res[0][0]
    # check constraint not satisfied before adding it
    assert sol.get_max_end_time() > max_time_target
    assert sol.get_max_end_time() < min_time_target

    # add constraint <= and check it
    solver.add_constraint_on_max_end_time(sign=SignEnum.LEQ, time=max_time_target)
    sol = solver.solve(
        callbacks=[NbIterationStopper(nb_iteration_max=1)]
    ).get_best_solution()
    assert sol.get_max_end_time() <= max_time_target
    ...
    solver.init_model()
    # add constraint and check it
    solver.add_constraint_on_max_end_time(sign=SignEnum.UEQ, time=min_time_target)
    sol = solver.solve(
        callbacks=[NbIterationStopper(nb_iteration_max=1)]
    ).get_best_solution()
    assert sol.get_max_end_time() >= min_time_target
