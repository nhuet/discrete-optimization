...
def problem_constraints(
    current_solution: Union[PreemptiveRcpspSolution, PreemptiveMultiskillRcpspSolution],
    ...
    multimode = isinstance(
        cp_solver, (CpMultimodePreemptiveRcpspSolver, CpMultiskillRcpspSolver)
    )
    if multimode:
    ...
    for job in subtasks:
        ...
        for k in range(len(list_starts), cp_solver.nb_preemptive):
            if isinstance(
                cp_solver, (CpMultiskillRcpspSolver, CpPreemptiveMultiskillRcpspSolver)
            ):
                string1_dur = cp_solver.constraint_duration_string_preemptive_i(
...
class NeighborRepairProblems(MznConstraintHandler):
    def __init__(
        ...
        if isinstance(self.problem, SpecialConstraintsPreemptiveRcpspProblem,) or (
            ...
            self.graph_rcpsp = GraphSpecialConstraintsRcpsp(problem=self.problem)
