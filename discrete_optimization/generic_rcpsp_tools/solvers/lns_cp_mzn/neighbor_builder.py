...
def return_random_basic_constraint_handler(
    ...
    return BasicConstraintBuilder(
        ...
        neighbor_builder=NeighborRandom(
            problem=rcpsp_problem, graph=graph, fraction_subproblem=fraction_to_fix
        ),
    )
...
def build_neighbor_random(
    option_neighbor: OptionNeighborRandom, rcpsp_problem: ANY_RCPSP
) -> MznConstraintHandler:
    graph = build_graph_rcpsp_object(rcpsp_problem=rcpsp_problem)
    params_use_case = [
        ConstraintHandlerScheduling(
            ...
            basic_constraint_builder=BasicConstraintBuilder(
                ...
                neighbor_builder=NeighborRandom(
                    problem=rcpsp_problem, graph=graph, fraction_subproblem=0.75
                ),
            ),
            params_list=[
                ParamsConstraintBuilder(
...
def build_neighbor_mixing_methods(
    rcpsp_problem: ANY_RCPSP, graph: GraphRcpsp = None, **kwargs
):
    if graph is None:
        graph = build_graph_rcpsp_object(rcpsp_problem=rcpsp_problem)
    n1 = NeighborBuilderSubPart(
        problem=rcpsp_problem, graph=graph, nb_cut_part=kwargs.get("cut_part", 10)
    )
    ...
    basic_constraint_builder = BasicConstraintBuilder(
        ...
        preemptive=rcpsp_problem.is_preemptive(),
        multiskill=isinstance(
            rcpsp_problem, (VariantMultiskillRcpspProblem, MultiskillRcpspProblem)
        ),
    )
    params_list = kwargs.get(
        "params_list",
...
def build_neighbor_mixing_cut_parts(
    ...
    n1_s = [
        NeighborBuilderSubPart(problem=rcpsp_problem, graph=graph, nb_cut_part=c_part)
    ...
    **kwargs
    ...
    a_s = [
        build_neighbor_random(
            option_neighbor=option_neighbor_random, rcpsp_problem=rcpsp_problem
        )
    ]
    b = build_neighbor_mixing_methods(rcpsp_problem, graph=graph, **kwargs)
    list_c = a_s + [b]
...
def random_neigh(rcpsp_problem, fraction_subproblem: float = 0.35, **kwargs):
    return build_basic_random_constraint_handler(
        ...
        preemptive=kwargs.get("preemptive", rcpsp_problem.is_preemptive()),
        multiskill=kwargs.get("multiskill", rcpsp_problem.is_multiskill()),
        use_makespan_of_subtasks=kwargs.get("use_makespan_of_subtasks", False),
        objective_subproblem=kwargs.get(
            "objective_subproblem", ObjectiveSubproblem.GLOBAL_MAKESPAN
        ),
...
def time_window_neigh(rcpsp_problem, **kwargs):
...
def constraint_neigh(rcpsp_problem, fraction_subproblem: float = 0.35, **kwargs):
    gr = build_graph_rcpsp_object(rcpsp_problem=rcpsp_problem)
    n1 = NeighborBuilderSubPart(
        problem=rcpsp_problem,
        graph=build_graph_rcpsp_object(rcpsp_problem=rcpsp_problem),
...
def cut_parts(rcpsp_problem, nb_cut_part=4, **kwargs):
...
def mix(rcpsp_problem, nb_cut_part=3, fraction_subproblem=0.25):
    c1 = cut_parts(rcpsp_problem, nb_cut_part)
    c2 = random_neigh(rcpsp_problem, fraction_subproblem=fraction_subproblem)
    return ConstraintHandlerMix(
        problem=rcpsp_problem,
...
def mix_lot(rcpsp_problem, nb_cut_parts, fraction_subproblems, **kwargs):
    c1 = [
        cut_parts(rcpsp_problem, nb_cut_part, **kwargs) for nb_cut_part in nb_cut_parts
    ]
    c2 = [
        random_neigh(rcpsp_problem, fraction_subproblem=fraction_subproblem, **kwargs)
    ...
    if (
        ...
        c3 = [constraint_neigh(rcpsp_problem, fraction_subproblem=0.25, **kwargs)]
        tags += ["generalized_precedence_constraint"]
    else:
        c3 = []
    c4 = []
    if "time_windows" in kwargs and kwargs["time_windows"]:
        c4 = [time_window_neigh(rcpsp_problem=rcpsp_problem, **kwargs)]
        tags += ["time_windows"]
    ...
    if kwargs.get("equilibrate_multiskill", False):
        if rcpsp_problem.is_preemptive():
            c5 = [
                EquilibrateMultiskillAllocation(problem=rcpsp_problem)
                for i in range(max(1, int((len(c1) + len(c2) + len(c3)) / 2)))
            ]
            tags += ["equilibrate_" + str(i) for i in range(len(c5))]
        else:
            c5 = [
                EquilibrateMultiskillAllocationNonPreemptive(problem=rcpsp_problem)
                for i in range(max(1, int((len(c1) + len(c2) + len(c3)) / 2)))
            ]
            tags += ["equilibrate_" + str(i) for i in range(len(c5))]
    if kwargs.get("equilibrate_multiskill_v2", False):
        c5 = [
            ConstraintHandlerMultiskillAllocation(problem=rcpsp_problem)
            for i in range(max(1, int((len(c1) + len(c2) + len(c3)) / 2)))
        ]
        tags += ["equilibrate_" + str(i) for i in range(len(c5))]
