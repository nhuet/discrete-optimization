...
def schedule_solution_to_variant(solution: MultiskillRcpspSolution):
    s: MultiskillRcpspSolution = solution
    ...
    for i in solution.problem.tasks_list_non_dummy:
        ...
        workers += [w]
    solution = VariantMultiskillRcpspSolution(
        problem=s.problem,
        priority_list_task=[
...
class VariantPreemptiveMultiskillRcpspSolution(PreemptiveMultiskillRcpspSolution):
    ...
    def copy(self):
        return VariantPreemptiveMultiskillRcpspSolution(
            problem=self.problem,
            priority_list_task=deepcopy(self.priority_list_task),
            modes_vector=deepcopy(self.modes_vector),
            priority_worker_per_task=deepcopy(self.priority_worker_per_task),
...
def schedule_solution_preemptive_to_variant(
    solution: PreemptiveMultiskillRcpspSolution,
):
    s: PreemptiveMultiskillRcpspSolution = solution
    ...
    for i in s.problem.tasks_list_non_dummy:
        ...
        w += [wi for wi in s.problem.employees if wi not in w]
        workers += [w]
    solution = VariantPreemptiveMultiskillRcpspSolution(
        problem=s.problem,
        priority_list_task=[
...
def sgs_multi_skill(solution: VariantMultiskillRcpspSolution):
    problem: MultiskillRcpspProblem = solution.problem
    activity_end_times = {}
...
def sgs_multi_skill_preemptive(solution: VariantPreemptiveMultiskillRcpspSolution):
    problem: MultiskillRcpspProblem = solution.problem
    activity_end_times = {}
    unfeasible_non_renewable_resources = False
    unfeasible_in_horizon = False
...
class MultiskillRcpspProblem(
    ...
    def __init__(
        ...
        if employees_list is None:
            ...
            assert set(employees_list) == set(self.employees), (
    ...
    def get_modes_dict(self, rcpsp_solution: MultiskillRcpspSolution):
        return rcpsp_solution.modes

    def evaluate_function(self, rcpsp_sol: MultiskillRcpspSolution):
    ...
    def satisfy(self, variable: Solution) -> bool:
        if isinstance(variable, PreemptiveMultiskillRcpspSolution):
...
class VariantMultiskillRcpspProblem(MultiskillRcpspProblem):
    def __init__(
        self,
        skills_set: set[str],
        resources_set: set[str],
        ...
        MultiskillRcpspProblem.__init__(
            self,
    ...
    def evaluate_function(self, rcpsp_sol: VariantMultiskillRcpspSolution):
        try:
...
def permutation_do_to_permutation_sgs_fast(
    rcpsp_problem: MultiskillRcpspProblem, permutation_do
):
    perm_extended = [
...
def priority_worker_per_task_do_to_permutation_sgs_fast(
    rcpsp_problem: MultiskillRcpspProblem, priority_worker_per_task
):
...
def cluster_employees_to_resource_types(ms_rcpsp_problem: MultiskillRcpspProblem):
...
def compute_overskill(
    problem: MultiskillRcpspProblem, solution: PreemptiveMultiskillRcpspSolution
):
    overskill = {}
    for task in problem.tasks_list:
        overskill[task] = {}
